...
#
# package.json's `postinstall` script runs `prisma generate`, which requires
# prisma/schema.prisma to already be present — so that file must be copied
# in BEFORE `npm install` runs, not just the two package manifests.
FROM base AS deps
